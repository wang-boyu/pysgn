...
def test_basic_network_creation(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    """
    g = small_world_network(point_gdf, k=2, p=0.1)
...
def test_k_as_column(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    g = small_world_network(point_gdf, k="expected_degree", p=0.1, query_factor=1)
...
def test_polygon_geometry(polygon_gdf: gpd.GeoDataFrame) -> None:
    ...
    g = small_world_network(polygon_gdf, k=2, p=0.1)
...
def test_custom_constraint(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    g = small_world_network(point_gdf, k=2, p=0.1, constraint=group_constraint)
...
def test_save_attributes(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    g1 = small_world_network(point_gdf, k=2, p=0.1, save_attributes="group")
    assert all("group" in g1.nodes[n] for n in g1.nodes)
    assert all("expected_degree" not in g1.nodes[n] for n in g1.nodes)

    g2 = small_world_network(point_gdf, k=2, p=0.1, save_attributes=["group"])
    ...
    g3 = small_world_network(point_gdf, k=2, p=0.1, save_attributes=True)
    ...
    g4 = small_world_network(point_gdf, k=2, p=0.1, save_attributes=False)
...
def test_max_degree_constraint(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    """
    max_degree = 3
    g = small_world_network(point_gdf, k=2, p=0.1, max_degree=max_degree)
...
def test_invalid_inputs(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    with pytest.raises(ValueError):
        small_world_network(point_gdf, k=1.5, p=0.1)

    # Test invalid probability
    with pytest.raises(ValueError):
        small_world_network(point_gdf, k=2, p=1.5)

    # Test invalid id column
    with pytest.raises(KeyError):
        small_world_network(point_gdf, k=2, p=0.1, id_col="nonexistent")
...
def test_geographic_warning(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    with pytest.warns(UserWarning):
        small_world_network(geographic_gdf, k=2, p=0.1)
...
def test_rewiring_probability(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    g1 = small_world_network(point_gdf, k=2, p=0.0)
    g2 = small_world_network(point_gdf, k=2, p=1.0)
...
def test_index_as_id(point_gdf: gpd.GeoDataFrame) -> None:
    ...
    g = small_world_network(point_gdf, k=2, p=0.1, id_col="index")
    ...
    g_custom = small_world_network(custom_gdf, k=2, p=0.1, id_col="index")
    ...
    with pytest.raises(ValueError, match="ID column must contain unique values"):
        small_world_network(dup_gdf, k=2, p=0.1, id_col="index")
    ...
    with pytest.raises(ValueError, match="Multi-index is not supported"):
        small_world_network(multi_gdf, k=2, p=0.1, id_col="index")
